...
void main() async {
  ...
  final pet2Json = '{"id": "2", "name": "Coder", "age": 1}';

  final pet2JsonMap = json.decode(pet2Json);

  // final pet2 = serializers.deserializeWith(Pet.serializer, pet2JsonMap);
  final pet2 = Pet.fromJson(pet2JsonMap);
  ...
  // final pet3JsonMap = serializers.serializeWith(Pet.serializer, pet3);
  final pet3JsonMap = pet3.toJson();

  final pet3Json = json.encode(pet3JsonMap);
  ...
  final pet4 = pet3.rebuild((b) => b..age = 5);
  ...
  logger.i('Gender.female.serialize()');
  logger.v(Gender.female.serialize());

  logger.i("Gender.deserialize('F')");
  logger.v(Gender.deserialize('F'));

  logger.i('Gender.male.serialize()');
  logger.v(Gender.male.serialize());

  logger.i("Gender.deserialize('male')");
  logger.v(Gender.deserialize('male'));
  ...
  try {
    final response =
        await http.get('https://jsonplaceholder.typicode.com/todos?_limit=2');

    if (response.statusCode == 200) {
      final todosJson = response.body;

      final List<dynamic> todosJsonList = json.decode(todosJson);

      final todos = todosJsonList.map((e) => Todo.fromJson(e)).toBuiltList();
      ...
      final todosJsonList2 = todos.map((e) => e.toJson()).toList();
      ...
      logger.e('Failed to fetch the json');
    }
  } catch (e) {
    logger.e(e);
  }
  ...
  try {
    final response = await http.get('https://randomuser.me/api/?results=2');

    if (response.statusCode == 200) {
      final randomUserResponseJson = response.body;

      final randomUserResponseJsonMap = json.decode(randomUserResponseJson);

      final randomUserResponse =
          RandomUserResponse.fromJson(randomUserResponseJsonMap);

      logger.i('randomUserResponseJson');
      logger.v(randomUserResponseJson);

      logger.i('randomUserResponse');
      logger.v(randomUserResponse);
    } else {
      logger.e('Failed to fetch the json');
    }
  } catch (e) {
